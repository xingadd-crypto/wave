...
constexpr UINT kTrayCmdOpen = 2001;
constexpr UINT kTrayCmdExit = 2002;
...
void Win32Window::AddTrayIcon() {
  ...
  nid.hIcon = LoadIconW(GetModuleHandle(nullptr), MAKEINTRESOURCE(IDI_APP_ICON));
  lstrcpynW(nid.szTip, L"Wave", ARRAYSIZE(nid.szTip));
...
void Win32Window::ShowTrayMenu() {
  if (tray_menu_ == nullptr) {
    ...
    AppendMenuW(tray_menu_, MF_STRING, kTrayCmdOpen,
                L"\x6253\x5f00\x4e3b\x754c\x9762");
    AppendMenuW(tray_menu_, MF_SEPARATOR, 0, nullptr);
    AppendMenuW(tray_menu_, MF_STRING, kTrayCmdExit, L"\x9000\x51fa");
  }
  ...
  UINT command = TrackPopupMenu(
      tray_menu_, TPM_RIGHTBUTTON | TPM_RETURNCMD | TPM_NONOTIFY, cursor.x,
      cursor.y, 0, window_handle_, nullptr);
  if (command == kTrayCmdOpen) {
    ShowWindow(window_handle_, SW_SHOWNORMAL);
    SetForegroundWindow(window_handle_);
  } else if (command == kTrayCmdExit) {
    RemoveTrayIcon();
    DestroyWindow(window_handle_);
  }
}
